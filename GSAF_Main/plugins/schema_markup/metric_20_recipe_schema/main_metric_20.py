...
Detects whether food-related pages use Recipe structured data.
'''
...
def run(site_data):
    result = MetricResult(factor="20 - Recipe Schema")
    try:
        blocks = site_data.json_ld
        has_recipe = any(_contains_type(b, "Recipe") for b in blocks)

        score = 100 if has_recipe else 0
        result.score = score
        result.status = get_status(score)
        result.details["ld_json_blocks_found"] = len(blocks)

        if not has_recipe:
            result.recommendations.append("Add Recipe schema if this page contains a food recipe.")
